...
"""
Functions obtained from MDFPtools: https://github.com/rinikerlab/mdfptools/
...
class ComposerGMX:
    ...
    def solute_solvent_split(self, topology, solute_residue_name = None):
        ...
        if isinstance(solute_residue_name, list):
            ...
            for res_name in solute_residue_name:
                ...
                #solute_atoms = solute_atoms + [atom.index for atom in topology.atoms if atom.residue.chain.index == res_name]
        else:
            solute_atoms = [atom.index for atom in topology.atoms if atom.residue.chain.index == solute_residue_name]
        ...
        solvent_atoms = [atom.index for atom in topology.atoms if atom.residue.name != solute_residue_name ]
    ...
    @classmethod
    def calc_shape_from_SMILES(self, molecule):
    ...
    @classmethod
    def calc_psa3d(self, obj_list=None, include_SandP = True, atom_to_remove = None):
        """
    ...
    @classmethod
    def extract_protein_ligang_energy_terms_single(self, csv_file = None, df_ene = pd.DataFrame(), cmpd_name = None, N_equil = None, return_df = False):
        """
        ...
        - Ligand - Water     Coulomb, Lennard-Jones and Total Intermolecular Energy (Only if present in the binding pocket)
        - Total Coulomb, Lennard-Jones and Total contributions: Sum of the previous terms

        Either csv_file or df_ene must be specified as input!!!

        Parameters:
        --------------------
        csv_file: str, optional
            csv file containing energy terms extracted from simulations using the gmx_energy function of GROMACS
        ...
        if csv_file == None and df_ene.empty:
            print("Error in extract_protein_ligang_energy_terms_single: Either csv_file or  must be specified as input")
            return

        # chech that the file exists
        if df_ene.empty and  csv_file != None:
            if csv_file != None and not os.path.isfile(csv_file):
                print("Error in extract_protein_ligang_energy_terms_single: File {} does not exists".format(csv_file))
                return
            else:
                # Read energy file
                df_ene = pd.read_csv(csv_file)
        ...
        idx_coul_lig = [i for i, s in enumerate(df_ene) if ('Coul' in s and 'Peptide-Peptide' in s)]
        idx_lj_lig = [i for i, s in enumerate(df_ene) if ('LJ' in s and 'Peptide-Peptide' in s)]
        idx_ene_lig = [i for i, s in enumerate(df_ene) if 'Peptide-Peptide' in s]
        ...
        # lig - protein
        idx_coul_pl = [i for i, s in enumerate(df_ene) if ('Coul' in s and 'Peptide-Receptor' in s)]
        idx_lj_pl = [i for i, s in enumerate(df_ene) if ('LJ' in s and 'Peptide-Receptor' in s)]
        idx_ene_pl = [i for i, s in enumerate(df_ene) if 'Peptide-Receptor' in s]
        ...
        # lig - water
        idx_coul_wl = [i for i, s in enumerate(df_ene) if ('Coul' in s and 'Peptide-Water' in s)]
        idx_lj_wl = [i for i, s in enumerate(df_ene) if ('LJ' in s and 'Peptide-Water' in s)]
        idx_ene_wl = [i for i, s in enumerate(df_ene) if 'Peptide-Water' in s]
        ...
        dict_ProtLig_ene = {'intra_crf_av_PL': intra_lig_crf[0], 'intra_crf_std_PL': intra_lig_crf[1], 'intra_crf_med_PL': intra_lig_crf[2], 'intra_lig_lj_av_PL': intra_lig_lj[0], 'intra_lig_lj_std_PL': intra_lig_lj[1], 'intra_lig_lj_med_PL': intra_lig_lj[2], 'intra_ene_av_PL': intra_lig[0], 'intra_ene_std_PL': intra_lig[1], 'intra_ene_med_PL': intra_lig[2], 'prot_lig_crf_av_PL': inter_prot_lig_crf[0], 'prot_lig_crf_std_PL':inter_prot_lig_crf[1], 'prot_lig_crf_med_PL':inter_prot_lig_crf[2], 'prot_lig_lj_av_PL':inter_prot_lig_lj[0], 'prot_lig_lj_std_PL':inter_prot_lig_lj[1], 'prot_lig_lj_med_PL':inter_prot_lig_lj[2], 'prot_lig_ene_av_PL':inter_prot_lig[0], 'prot_lig_ene_std_PL':inter_prot_lig[1], 'prot_lig_ene_med_PL':inter_prot_lig[2], 'tot_crf_av_pl': inter_tot_crf[0], 'tot_crf_std_pl':inter_tot_crf[1], 'tot_crf_med_pl':inter_tot_crf[2], 'tot_lj_av_pl':inter_tot_lj[0], 'tot_lj_std_pl':inter_tot_lj[1], 'tot_lj_med_pl':inter_tot_lj[2], 'tot_ene_av_pl':inter_tot[0], 'tot_ene_std_pl':inter_tot[1], 'tot_ene_med_pl':inter_tot[2]}
